...
//
// Every quirk found in the earlier probes is applied here rather than rediscovered.
...
// --- bd adapter -------------------------------------------------------------
// Beads probe: bd init blocks on an interactive prompt; list/show carry no
...
function runSensors() {
  ...
  for (const s of SENSORS) {
    ...
    try {
      ...
      // Sensor output is untrusted. Cap it and strip control characters
...
if (cmd === "init") {
  // Beads probe: bd init asks "Contributing to someone else's repo? [y/N]" and waits
  ...
  // Beads probe: one atomic call. Selecting and owning must not be two steps.
