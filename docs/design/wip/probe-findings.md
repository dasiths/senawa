...
| 48 | Browser commands can be acknowledged durably before execution and recovered after supervisor restart | **Confirmed offline in the production HTTP path and real-Beads composition.** A `SIGKILL` contract recovered one durably queued command after lease expiry without duplicate transitions; receipt-local SSE replay, reconnect, backpressure, and independent-writer polling also passed |
