...
describe("portal state", () => {
  ...
  it("keeps authority caches revision keyed and locks actions through gaps", () => {
    const original = initialPortalState(parsePortalHash("#/runs/repository_one/run_one/timeline"));
  ...
  it("deduplicates events and caps the visible ring at 500", () => {
    let state = initialPortalState({ name: "timeline" });
  ...
  it("closes reviewed authority when the selected run changes", () => {
    const original = portalReducer(initialPortalState({ name: "timeline" }), {
      type: "dialog-open",
      dialog: {
        kind: "end",
        title: "End this run",
  ...
  it("retains pending recovery identity when a session expires", async () => {
    ...
    const withPending = portalReducer(initialPortalState({ name: "timeline" }), {
      type: "pending-add",
      pending,
    });
  ...
  it("discards retained transcript lines when the run, route, or stream changes", () => {
    ...
    for (const action of [
      ...
      { type: "route", route: { name: "timeline" } },
